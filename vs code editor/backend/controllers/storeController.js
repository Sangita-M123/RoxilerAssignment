...
const listStores = async (req, res) => {
  try {
    ...
    const where = query ? {
      [Op.or]: [{ name: { [Op.like]: `%${query}%` } }, { address: { [Op.like]: `%${query}%` } }]
    } : {};
    const stores = await Store.findAll({ where, include: ['owner'] });
    res.json(stores);
  } catch (err) {
...
const getStore = async (req, res) => {
  try {
    const store = await Store.findByPk(req.params.id, { include: ['owner'] });
